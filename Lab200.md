...
- 빌드한 Docker Image를 Registry에 등록하기 위해서는 접속정보가 필요합니다. Registy 별로 약간의 차이는 있지만, internal/docker-push step으로 OCI Registry는 다음 정보들이 필요합니다.
    * Regitry 종류별 가이드는 다음 링크를 참고하세요.
    ![Pushing Images](https://devcenter.wercker.com/administration/containers/pushing-images/)
  ```
  username: $OCI_USERNAME
  password: $OCI_AUTH_TOKEN
  repository: $OCI_REPOSITORY_NAME
